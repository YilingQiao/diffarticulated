...
struct MeshcatUrdfVisualizer {
  ...
  void convert_link_visuals(TinyUrdfLink &link, int link_index,
    ...
    for (int vis_index = 0; vis_index < (int)link.urdf_visual_shapes.size();
      ...
      if (v.geometry.geom_type == TINY_MESH_TYPE) {
        ...
        std::string obj_data;
        std::cout << m_path_prefix.c_str() << " mesh name " << (m_path_prefix + v.geometry.m_mesh.m_file_name).c_str() << "\n";
        v.geometry.m_mesh.m_scale.print("m_scale");
        ...
        if (fp) {
          ...
          printf("fail to open mesh\n");
        }
        int str_len = obj_data.length();

        double world_scale[3] = {
          TinyConstants::getDouble(v.geometry.m_mesh.m_scale[0]), 
          TinyConstants::getDouble(v.geometry.m_mesh.m_scale[1]), 
          TinyConstants::getDouble(v.geometry.m_mesh.m_scale[2])
        };
        ...
        if (str_len) {
          ...
          if (useTextureUuid) { 
            ...
            nlohmann::json cmd = create_textured_mesh_cmd(
                obj_data_utf8.c_str(), m_texture_data.c_str(), world_pos, world_scale, 
                color_rgb, vis_name.c_str());


            std::string save_name =  "." + vis_name + ".json";
            save_name.insert(10, "obj/");
            // std::string save_name = "./objs/" + vis_name + ".json";
  ...
  void sync_visual_transforms(const TinyMultiBody *body, int step = 0) {
    // sync base transform
    for (int v = 0; v < body->m_visual_uids2.size(); v++) {
      ...
      if (m_b2vis.find(visual_id) != m_b2vis.end()) {
        ...
        double world_mat[9] = {
          TinyConstants::getDouble(m.getRow(0)[0]), 
          TinyConstants::getDouble(m.getRow(1)[0]), 
          TinyConstants::getDouble(m.getRow(2)[0]),
          TinyConstants::getDouble(m.getRow(0)[1]), 
          TinyConstants::getDouble(m.getRow(1)[1]), 
          TinyConstants::getDouble(m.getRow(2)[1]),
          TinyConstants::getDouble(m.getRow(0)[2]), 
          TinyConstants::getDouble(m.getRow(1)[2]), 
          TinyConstants::getDouble(m.getRow(2)[2])};

        nlohmann::json tr_cmd =
            create_transform_cmd(world_pos, world_mat, viz.vis_name.c_str());


        std::string save_name =  "." + viz.vis_name + ".json";
        save_name.insert(10, "his/" + std::to_string(step) + "_");
        std::ofstream file(save_name);
        file << tr_cmd;
    ...
    for (int l = 0; l < body->m_links.size(); l++) {
      for (int v = 0; v < body->m_links[l].m_visual_uids2.size(); v++) {
        ...
        if (m_b2vis.find(visual_id) != m_b2vis.end()) {
          ...
          // std::cout << world_pos << "link world_pos\n";
          nlohmann::json tr_cmd =
              create_transform_cmd(world_pos, world_mat, viz.vis_name.c_str());
