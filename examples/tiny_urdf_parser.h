...
struct TinyUrdfParser {
  ...
  static bool parse_inertia(TinyUrdfInertial& urdf_inertial,
    ...
    const tinyxml2::XMLElement* arm_xml = config->FirstChildElement("armature");
    if (arm_xml) {
      urdf_inertial.armature =
          TinyConstants::scalar_from_string(arm_xml->Attribute("value"));
    }
  ...
  static bool parse_geometry(TinyUrdfGeometry& geom,
    ...
    if (type_name == "sphere") {
      ...
      if (shape->Attribute("scale")) {
        ...
        geom.m_mesh.m_scale.print("scale");
        // std::cout << geom.m_mesh.m_scale << "\n";
      }
    ...
    // std::cout << "parse geometry " << geom.geom_type << "\n";
  ...
  static bool parse_link(TinyUrdfLink& link,
    ...
    for (const tinyxml2::XMLElement* col_xml =
      ...
      if (parse_collision(col, col_xml, logger)) {
        ...
        std::cout << "parse collision " << col.geometry.geom_type << std::endl;
      } else {
